...
class EquipementController extends AbstractController
{
    ...
    public function generatePdf(EquipementRepository $equipementRepository)
    {
        $pdfoption = new Options();
        $pdfoption->set('defaultFont','system-ui');
        $dompdf = new Dompdf($pdfoption);
        $equipements = $equipementRepository->findAll();
        $html = $this->renderView('equipement/pdfequipement.html.twig', [
            'equipements' => $equipements
        ]);
        $dompdf->loadHtml($html);
        $dompdf->setPaper('A4','portrait');
        $dompdf->render();
        $fichier = 'equipements-liste'.'.pdf';
        $dompdf->stream($fichier, [
            'Attachment' => true
        ]);
        return new Response();
    }
    #[Route('/stats', name: 'app_equipement_stats')]
        public function stats(EquipementRepository $equipementRepository): Response
        {
            ...
            // Group equipment by quantity ranges
            $ranges = [
                '0-10' => 0,
                '11-50' => 0,
                '51-100' => 0,
            ...
            foreach ($equipements as $equipement) {
                $qty = $equipement->getQuantiteDisponible();
